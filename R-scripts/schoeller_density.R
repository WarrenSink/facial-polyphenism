...
schoeller_cf_negctrl_scans <- schoeller_cf_negctrl_scans %>% 
  ...
  .[,c(21,2:4)] %>%
  left_join(., all_dist_pheno[1:116,c(1:9)], by = c("target" = "Names")) %>% 
  left_join(., all_dist_pheno[1:116,c(1:9)], by = c("source" = "Names")) %>%
  distinct()
...
arranged_adp <- all_dist_pheno %>%
  arrange(as.integer(as.character(Co_twins_ID))) 

schoeller_cf <- schoeller_cf %>%
  select(target, source, Similarity) %>%
  left_join(., arranged_adp[1:116,1:9], by = c("target" = "Names")) %>% 
  left_join(., arranged_adp[1:116,1:9], by = c("source" = "Names")) %>%
  distinct() 

id.List <- list()
for (i in 1:nrow(schoeller_cf)){
  if (schoeller_cf[i,5] == schoeller_cf[i,13]){
    id.List[[i]] = "co-twins"
  }
  else{
...
schoeller_cf_negctrl = schoeller_cf_negctrl %>%
  select(target, source, Similarity) %>%
  left_join(., arranged_adp[1:116,c(1:9)], by = c("target" = "Names")) %>% 
  left_join(., arranged_adp[1:116,c(1:9)], by = c("source" = "Names")) %>%
  distinct() 
...
schoeller_cf$ctrl_id <- as.factor(schoeller_cf$ctrl_id)
schoeller_cf <- schoeller_cf %>% arrange(factor(ctrl_id, levels(ctrl_id)[c(1,2,3)]))

write.csv(schoeller_cf, "/Users/Warren.Sink/github/facial-polyphenism/CSVs/schoeller_cf.csv", row.names=FALSE)
